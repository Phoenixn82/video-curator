...
def main() -> int:
    ...
    ap.add_argument("--max-frames", type=int, default=80, help="Cap on frame count (default 80, hard max 100). Ignored if --frames-per-hour is set.")
    ap.add_argument("--frames-per-hour", type=int, default=30, help="Target frames per hour of video (default 30). Overrides --max-frames when set; no upper cap.")
    ...
    if args.frames_per_hour is not None and args.frames_per_hour > 0:
        max_frames = None  # computed from duration after metadata probe
    else:
    ...
    if max_frames is None:
        # Density mode: derive cap from duration × frames-per-hour
        hours = max(effective_duration / 3600.0, 1.0 / 60.0)  # min 1-min equivalent
        max_frames = max(1, int(round(hours * args.frames_per_hour)))
        print(f"[watch] frames-per-hour mode: {args.frames_per_hour} fph × {hours:.2f}h = {max_frames} frame budget", file=sys.stderr)
